...
void setup_and_recover() {
    ...
    if (!std::filesystem::is_directory(DATA_PATH) || !std::filesystem::exists(DATA_PATH)) { // Check if src folder exists
    ...
    for (string tbl_name: table_names) {
        string tbl_path = DATA_PATH + tbl_name + ".tbl";
        string tbl_scm_path = DATA_PATH + tbl_name + ".scm";
